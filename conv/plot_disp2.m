function ans = plot_disp(r)
   ...
   if r == 1
      file = ['run12_rpc_seed' rn '/res-D_sub.tif'];
      H = figure(1); set(H, 'Position', [0   400 500 500]);
   else
      file = ['run12_rpc_seed' rn '/res-D_sub2.tif'];
...
%
%   if r ~= 1
%      file = ['nomap_seed' rn '_subpix2/res-D_sub2_row2.tif'];
%      A=imread(file);
%      X = A(:, :, 1);
%      disp(sprintf('min max X %g %g', min(min(X)), max(max(X))));
%      X = min(X, b); X = max(X, -b);
%      subplot(2, 2, 4); imagesc(X); colorbar; title('is valid')
%   end
%   [m, n] = size(V);
%   I = find(V ~= 0);
%   XV = X(I); YV = Y(I);
