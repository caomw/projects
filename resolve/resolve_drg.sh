...
if [ "$#" -lt 4 ]; then echo Usage: $0 mpp doOrtho left right;  exit; fi
...
d=0.0380
outDir="results_"$mpp"mpp"_"$a"_"$b""_run4"
...
if [ $doOrtho -eq  0 ]; then
    time_run.sh stereo --compute-error-vector -s stereo.default --threads 16 $left_sc $right_sc $res --nodata-threshold $d # 0.01 #0.02 # 0.025 # 0.038 #  --nodata-optimal-threshold-factor 0.1 
    ...
    time_run.sh  point2dem --errorimage --dem-spacing $mpp     --nodata-value -32767 --threads 1 -r moon $res-PC.tif  --proj-lon 0 --proj-lat -90 --proj-scale 1 
    
    time_run.sh show_dems.pl $res-DEM.tif
    time_run.sh remote_copy.pl $res-DEM.tif $L2

    pref=$res-DEMError
    echo Will write: $pref.txt
    time_run.sh gdalinfo -stats $pref.tif >  $pref.txt
    time_run.sh gdalinfo -hist  $pref.tif >> $pref.txt
    time_run.sh gdal_translate -outsize 10% 10% $pref.tif $pref"_sub10".tif
fi
