function ans = plot_disp(r)
   ...
   if r == 1
      file = ['map_seed' rn '_subpix2/res-D.tif'];
      H = figure(1); set(H, 'Position', [0   400 500 500]);
   else
      %file = ['map_seed' rn '_subpix2/res-D.tif'];
      %file = ['map_seed' rn '_subpix2/res-D_sub2_col.tif'];
      file = ['map_seed' rn '_subpix2/res-D_sub.tif'];
...
%   
   ...
   if r ~= 1
      file = ['map_seed' rn '_subpix2/res-D_sub2.tif'];
      A=imread(file);
      X = A(:, :, 1);
      disp(sprintf('min max X %g %g', min(min(X)), max(max(X))));
      X = min(X, b); X = max(X, -b);
