function ans = plot_disp(r)
   ...
   if r == 1
      file = ['nomap_seed' rn '_subpix2/res-D.tif'];
      H = figure(1); set(H, 'Position', [0   400 500 500]);
   else
      %file = ['nomap_seed' rn '_subpix2/res-D.tif'];
      %file = ['nomap_seed' rn '_subpix2/res-D_sub2_col.tif'];
      file = ['nomap_seed' rn '_subpix2/res-D_sub.tif'];
...
%   
   ...
   if r ~= 1
      file = ['nomap_seed' rn '_subpix2/res-D_sub2.tif'];
      disp(sprintf('reading %s', file));
      A=imread(file);
      X = A(:, :, 1);
      disp(sprintf('min max X %g %g', min(min(X)), max(max(X))));
      X = min(X, b); X = max(X, -b);
      subplot(2, 2, 4); imagesc(X); colorbar; title('sampled')
   end
%   [m, n] = size(V);
%   I = find(V ~= 0);
%   XV = X(I); YV = Y(I);
