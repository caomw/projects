...
class Visualization(QtGui.QWidget):
    def __init__(self,argv):
        ...
        h_file = argv[1][:-4]+"-H.tif"
        print('Will plot ' + h_file)
        self.disparity_h_image.append(QtGui.QImage(h_file))
        v_file = argv[1][:-4]+"-V.tif"
        print('Will plot ' + v_file)
        self.disparity_v_image.append(QtGui.QImage(v_file))
        self.offset.append( QtCore.QPoint(0,0) )

        f = open(argv[1][:-4]+"_zone.txt", 'r')
        for line in f:
