...
src="--corr-search $a $b $c $d"
opt="$win_opt $src --corr-max-levels $levels -t dg -s stereo.default --threads 1 --alignment-method none --subpixel-mode $subpix --disable-fill-holes"
if [ $sub -ne 1 ]; then
...
#     ((dy=s-q))
    ...
    gdal_translate -projwin $p $q $r $s $img1_sub $img1_crop
    gdal_translate -projwin $p $q $r $s $img2_sub $img2_crop
