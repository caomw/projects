...
*******************************************************************************/
    ...
    case 2:
        ...
        if (lond < 0) {           // Test for negative (East) longitude
            sign_lon = -1.0;
            lond    *= -1.0;
        }
        *lat_dd = latd * sign_lat;
        *lon_dd = lond * sign_lon;
        if (*lon_dd < 0) *lon_dd *= -1.0;
        break;
    ...
    case 4:
        ...
        *lat_dd = (latm/60.0 + latd) * sign_lat;
        *lon_dd = (lonm/60.0 + lond) * sign_lon;
        if (*lon_dd < 0) *lon_dd *= -1.0;
        break;
    ...
    case 6:
        ...
        *lat_dd = ((lats/60.0 + latm)/60.0 + latd) * sign_lat;
        *lon_dd = ((lons/60.0 + lonm)/60.0 + lond) * sign_lon;
        if (*lon_dd < 0) *lon_dd *= -1.0;
        break;
