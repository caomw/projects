...
// $Date::               $
// $Id:: intg.c 54161 20#$
static const char PGMVER[]="3.12";
static const char PGMDAT[]="2011/05/02";
...
Which geoid model do you wish to use?\n\n       \
  ...
  99 = END PROGRAM\n\n                          \
   -> ");
    ...
    strncpy(dirnam, "C:\\Users\\oleg\\Geoid\\\0", 256);
...
Which longitude convention will you use? \n\
    ...
    if (iinput == 2) {
        while (iii == 0) {
            ...
            //trim_c( ifname, 'b' );
            strncpy(ifname, "latlon.txt\0", 256);
            printf("---File is %s\n", ifname);
    ...
    if (strcmp(ofyn, "Y") == 0 || strcmp(ofyn, "y") == 0) {
        while (iii == 0) {
            ...
            //trim_c( ofname, 'b');
            strncpy(ofname, "latlon_out.txt\0", 256);
            printf("---Out file is %s\n", ofname);

            if ((ofp = fopen(ofname, "w")) == NULL) {
              printf("Error: File exists, will overwrite\n");
              fprintf(ofp, "");
              ++iii;              
            } else {
              ++iii;
