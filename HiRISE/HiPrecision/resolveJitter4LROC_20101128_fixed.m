function [offx, offy ] = resolveJitter4LROC_20101128_fixed( imageid, rows , lineOffset, lineTime)
%resolveJitter4LROC_20101128('M139291998', 1044, 64.49, 0.00057653)
% function [ jitterx2, jittery2, ET, error] = resolveJitter4LROC( filepath1, lineinterval, time )
...
% pixels, ET (ephemeris time at that translation), average error between
...
tolcoef=.00001;
%tolcoef=.01;  %disp(sprintf('----- fix here!!!!'));
...
%% Filter out the bad points.
 ...
 disp(sprintf('window width is %g', windowWidth));
 disp(sprintf('size of out of range is %g', length(outofRange)));
 %return
 
%P = [AveOffX AveOffY];
%Q = load('out.txt');
%disp(sprintf('max err is %g', max(max(abs(P-Q)))));
%return
...
% number of columns is the same as nfft.
   tt = (0:(nfft-1))/(nfft);
% ET is a column vector that converts the even spacing to line number or
% line time, depending on what data type is read in. It has nfft rows.
ET = (0:(nfft-1))/(nfft-1)*duration+t01;


% The original data points, 'offx' or 'offy' interpolated at the original time or 
% line number 't', are interpolated over the evenly spaced 'ET' which 
...
% 't01' is subtracted from both 't' and 'ET' to set the starting point to zero. 
xinterp = interp1(t - t01, offx, ET-t01, 'pchip', mean(offx));
yinterp = interp1(t - t01, offy, ET-t01, 'pchip', mean(offy));
...
%P = load('out.txt');
%max(max(abs(P - overyy)))/max(max(abs(overyy)))
%return
...
%    return;
    ...
    disp(sprintf('---- fixed errror!!!'));
    aveerror=mean(1/2 * ( abs(xinterp - (jittercheckx+X(1)/2)) + abs(yinterp - (jitterchecky+Y(1)/2)) )  );
    %aveerror=mean(1/6 * (abs(xinterp - (jittercheckx+X(1)/2)))+ (abs(yinterp - (jitterchecky+Y(1)/2))));

    %tolcoef
    %disp(sprintf('put back the error formula!!!!!!!!!!!!!'));
    %aveerror=mean(1/6 * (abs(xinterp - (jittercheckx+X(1)/2)) +  abs(yinterp - (jitterchecky+Y(1)/2))));

    errors(k)=aveerror;

    disp(sprintf('error 1: %0.20g', aveerror));
    
%    P = [jittercheckx' jitterchecky'];
%    Q = load('out.txt');
%    size(P)
%    size(Q)
%    max(max(abs(P - Q)))/max(max(abs(P)))
...
%     figure(k)
%     clf
%     subplot(2,2,1), hold on, plot(t-tplot,offx,'.r'), plot(ET-tplot,xinterp,'g'),
%     plot(ET-tplot,jittercheckx+X(1)/2,'y'),hold off
%     subplot(2,2,2), hold on, plot(t-tplot,offy,'.r'), plot(ET-tplot,yinterp,'g'),
%     plot( ET-tplot, jitterchecky + Y(1) / 2,'y'),hold off
%     subplot(2,2,3), hold on, plot(ET - tplot , jitterx) , hold off
%     subplot(2,2,4), hold on, plot(ET - tplot , jittery) , hold off
%     figure(k+100)
%   tplot=t01;
%    clf
%     subplot(2,2,1), hold on, plot(t2-t02,offx2,'.r'), plot(ET1-tplot,xinterp2,'g'),
%     plot(ET1-tplot,jittercheckx2+X2(1)/2,'y'),hold off
%     subplot(2,2,2), hold on, plot(t2-t02,offy2,'.r'), plot(ET1-tplot,yinterp2,'g'),
%     plot( ET1-tplot, jitterchecky2 + Y2(1) / 2,'y'),hold off
%     subplot(2,2,3), hold on, plot(ET1 - tplot , jitterx) , hold off
%     subplot(2,2,4), hold on, plot(ET1 - tplot , jittery) , hold off
%     figure(k+200)
%     tplot=t01;
%     clf
%     subplot(2,2,1), hold on, plot(t3-t03,offx3,'.r'), plot(ET1-tplot,xinterp3,'g'),
%     plot(ET1-tplot,jittercheckx3+X3(1)/2,'y'),hold off
%     subplot(2,2,2), hold on, plot(t3-t03,offy3,'.r'), plot(ET1-tplot,yinterp3,'g'),
%     plot( ET1-tplot, jitterchecky3 + Y3(1) / 2,'y'),hold off
%     subplot(2,2,3), hold on, plot(ET1 - tplot , jitterx) , hold off
%     subplot(2,2,4), hold on, plot(ET1 - tplot , jittery) , hold off
    clear jittercheckx 
    clear jitterchecky 
...
%    figure(k+300)
%    clf
%    subplot(2,2,1), hold on, plot(t1-tplot,offx1,'.r'), plot(ET1-tplot,xinterp1,'g'),
%    plot(ET1-tplot,jittercheckx1+X1(1)/2,'y'),hold off
%    subplot(2,2,2), hold on, plot(t1-tplot,offy1,'.r'), plot(ET1-tplot,yinterp1,'g'),
%    plot( ET1-tplot, jitterchecky1 + Y1(1) / 2,'y'),hold off
%    subplot(2,2,3), hold on, plot(ET1-tplot,jitterx(k,:)), hold off
%    subplot(2,2,4), hold on, plot(ET1-tplot,jittery(k,:)), hold off
%    pause(0.01);
    ...
    disp(sprintf('error 2: %0.20g', aveerror));
end
...
%    clf
%    subplot(2,2,1), hold on, plot(t-tplot,offx,'.r'), plot(ET-tplot,xinterp,'g'),
%    plot(ET-tplot,jittercheckx+X(1)/2,'y'), xlabel('Image Time (s)'), ylabel('Pixels'), title('Sample Offsets'), hold off, grid on
%    subplot(2,2,2), hold on, plot(t-tplot,offy,'.r'), plot(ET-tplot,yinterp,'g'),
%    plot( ET-tplot, jitterchecky + Y(1) / 2,'y'), xlabel('Image Time (s)'), ylabel('Pixels'), title('Line Offsets'), hold off, grid on
%    
%     subplot(3,6,[3 4]), hold on, plot(t2-tplot,offx2,'.r'), plot(ET1-tplot,xinterp2,'g'),
%     plot(ET1-tplot,jittercheckx2+X2(1)/2,'y'),hold off, grid on 
%     subplot(3,6,[9 10]), hold on, plot(t2-tplot,offy2,'.r'), plot(ET1-tplot,yinterp2,'g'),
%     plot( ET1-tplot, jitterchecky2 + Y2(1) / 2,'y'),hold off, grid on
%     
%     subplot(3,6,[5 6]), hold on, plot(t3-tplot,offx3,'.r'), plot(ET1-tplot,xinterp3,'g'),
%     plot(ET1-tplot,jittercheckx3+X3(1)/2,'y'),hold off, grid on 
%     subplot(3,6,[11 12]), hold on, plot(t3-tplot,offy3,'.r'), plot(ET1-tplot,yinterp3,'g'),
%     plot( ET1-tplot, jitterchecky3 + Y3(1) / 2,'y'),hold off, grid on
%     
%    subplot(2,2,3), hold on, plot(ET-tplot,jitterx2), xlabel('Image Time (s)'), ylabel('Pixels'), title('Jitter Function - Sample Direction'), hold off, grid on 
%    subplot(2,2,4), hold on, plot(ET-tplot,jittery2), xlabel('Image Time (s)'), ylabel('Pixels'), title('Jitter Function - Line Direction'), hold off, grid on
%
...
data=[jitterx2,jittery2,ET];

fid=fopen([imageid,'_jitter_m_fixed.txt'], 'wt');
disp(sprintf('Writing: %s', [imageid,'_jitter_m_fixed.txt']));
